...
def draw(seq, data, path=None, src_path=None):  
    '''
    seq: game clips
    data: datasets
    '''
    ...
    seq = seq.decode("utf-8") + ".png"
        
    if not path:
        plt.savefig(seq, format='png', dpi=1000)
        
    else:
        plt.savefig(os.path.join(path, seq), format='png', dpi=1000)
